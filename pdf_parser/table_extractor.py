...
def extract_table(
    ...
    y_threshold: float = TableExtractor.DEFAULT_Y_THRESHOLD,
    x_threshold: float = TableExtractor.DEFAULT_X_THRESHOLD,
    align_columns_x: list = TableExtractor.DEFAULT_ALIGN_COLUMNS_X,
    align_columns_y: list = TableExtractor.DEFAULT_ALIGN_COLUMNS_Y
) -> pd.DataFrame:
    """
    ...
    df = TableExtractor.align_text_positions(pdf_data_subset, x_threshold, y_threshold, align_columns_x, align_columns_y)
    df = TableExtractor.sort_text_by_position(df)
    rows = TableExtractor.group_rows(df, y_threshold)
    normalized_cols, sorted_columns = TableExtractor.determine_column_positions(df, x_threshold)
    return TableExtractor.build_table(rows, normalized_cols, sorted_columns)
...
if __name__ == "__main__":
    ...
    extracted_table = extract_table(df_subset)
